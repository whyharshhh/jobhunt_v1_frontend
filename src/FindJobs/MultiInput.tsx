...
import { PillGroup } from '@mantine/core';
...
const MultiInput = (props:any) => {
  ...
  const options = data.filter((item) => item.toLowerCase().includes(search.trim().toLowerCase()))
    ...
    <Combobox.Option value={item} key={item} active={value.includes(item)}>
      <Group gap="sm">
        <Checkbox size='xs' color='brightSun.4' radius='xl'
    ...
    <Combobox store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
      <Combobox.DropdownTarget>
        <PillsInput 
          variant='unstyled' 
          leftSection={
            <div className='mr-2 rounded-full text-bright-sun-400 p-1 bg-mine-shaft-900'>
              <props.icon />
            </div>
          } 
